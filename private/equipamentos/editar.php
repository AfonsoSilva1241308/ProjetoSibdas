...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        $isComponente = isset($_POST['is_componente']) ? 'sim' : 'nao';
        ...
        $stmt->execute([
            ...
            ':marca'          => $_POST['marca'],
            ':modelo'         => $_POST['modelo'],
            ':num_serie'      => null_if_empty($_POST['numero_serie'] ?? ''),
            ':fabricante'     => $_POST['fabricante'],
            ':data_aquisicao' => null_if_empty($_POST['data_aquisicao'] ?? ''),
            ':tipo_entrada'   => null_if_empty($_POST['tipo_entrada'] ?? ''),
            ':ano_fabrico'    => null_if_empty($_POST['ano_fabrico'] ?? ''),
            ':custo'          => null_if_empty($_POST['custo'] ?? ''),
            ':estado'         => $_POST['estado'],
            ':criticidade'    => $_POST['criticidade'],
            ':servico'        => $_POST['servico'],
            ':sala'           => $_POST['sala'],
            ':observacoes'    => null_if_empty($_POST['observacoes'] ?? ''),
            ':is_componente'  => $isComponente,
            ':equipamento_pai_id' => ($isComponente === 'sim') ? null_if_empty($_POST['equipamento_pai'] ?? '') : null,
            ':inicio_garantia' => null_if_empty($_POST['inicio_garantia'] ?? ''),
            ':fim_garantia'   => null_if_empty($_POST['fim_garantia'] ?? ''),
            ':contrato_manutencao' => null_if_empty($_POST['contrato_manutencao'] ?? ''),
            ':entidade_responsavel' => null_if_empty($_POST['entidade_responsavel'] ?? ''),
            ':periodicidade'  => null_if_empty($_POST['periodicidade'] ?? ''),
            ':id'             => $idEquipamento
        ]);
...
<div class="d-flex vh-100">
    ...
    <div class="card w-100 shadow-sm rounded border-top border-primary border-4 h-auto" style="max-width: 1200px;">
        <div class="card-body p-4 p-md-5">
            ...
            <input type="hidden" name="id_escondido" value="<?= e($idEncrypted) ?>">
...
<div class="col-md-4">
    ...
    <select name="categoria" class="form-select border-0 bg-light shadow-sm">
        <option value="Monitorização" <?= selected_option($equipamento->categoria, 'Monitorização') ?>>Monitorização</option>
        <option value="Suporte de Vida" <?= selected_option($equipamento->categoria, 'Suporte de Vida') ?>>Suporte de Vida</option>
        <option value="Terapia" <?= selected_option($equipamento->categoria, 'Terapia') ?>>Terapia</option>
        <option value="Diagnóstico" <?= selected_option($equipamento->categoria, 'Diagnóstico') ?>>Diagnóstico</option>
    </select>
</div>
                            <div class="col-md-4">
...
<div class="col-md-4">
    ...
    <select name="tipo_entrada" class="form-select border-0 bg-light shadow-sm">
        <option value="Compra" <?= selected_option($equipamento->tipo_entrada, 'Compra') ?>>Compra</option>
        <option value="Doação" <?= selected_option($equipamento->tipo_entrada, 'Doação') ?>>Doação</option>
        <option value="Aluguer" <?= selected_option($equipamento->tipo_entrada, 'Aluguer') ?>>Aluguer</option>
        <option value="Empréstimo" <?= selected_option($equipamento->tipo_entrada, 'Empréstimo') ?>>Empréstimo</option>
    </select>
</div>
...
<div class="col-md-4">
    ...
    <select name="estado" class="form-select border-success shadow-sm" required>
        <option value="Ativo" <?= selected_option($equipamento->estado, 'Ativo') ?>>Ativo</option>
        <option value="Em Manutenção" <?= selected_option($equipamento->estado, 'Em Manutenção') ?>>Em Manutenção</option>
        <option value="Inativo" <?= selected_option($equipamento->estado, 'Inativo') ?>>Inativo</option>
        <option value="Abatido" <?= selected_option($equipamento->estado, 'Abatido') ?>>Abatido</option>
        <option value="Em Calibração" <?= selected_option($equipamento->estado, 'Em Calibração') ?>>Em Calibração</option>
    </select>
</div>
                        </div>
    ...
    <select name="criticidade" class="form-select border-warning shadow-sm" required>
        <option value="Baixa" <?= selected_option($equipamento->criticidade, 'Baixa') ?>>Baixa</option>
        <option value="Média" <?= selected_option($equipamento->criticidade, 'Média') ?>>Média</option>
        <option value="Alta" <?= selected_option($equipamento->criticidade, 'Alta') ?>>Alta</option>
        <option value="Suporte de Vida" <?= selected_option($equipamento->criticidade, 'Suporte de Vida') ?>>Suporte de Vida</option>
    </select>
</div>
...
<div class="col-md-4">
    ...
    <select name="servico" class="form-select border-0 bg-light shadow-sm" required>
        ...
        <option value="Urgência Geral" <?= selected_option($equipamento->servico, 'Urgência Geral') ?>>Urgência Geral</option>
        <option value="Cuidados Intensivos (UCI)" <?= selected_option($equipamento->servico, 'Cuidados Intensivos (UCI)') ?>>Cuidados Intensivos (UCI)</option>
        <option value="Bloco Operatório" <?= selected_option($equipamento->servico, 'Bloco Operatório') ?>>Bloco Operatório</option>
        <option value="Imagiologia" <?= selected_option($equipamento->servico, 'Imagiologia') ?>>Imagiologia</option>
        <option value="Internamento" <?= selected_option($equipamento->servico, 'Internamento') ?>>Internamento</option>
    </select>
</div>
...
<div class="col-md-4">
    ...
    <select name="sala" class="form-select border-0 bg-light shadow-sm" required>
        ...
        <option value="Box 1" <?= selected_option($equipamento->sala, 'Box 1') ?>>Box 1</option>
        <option value="Box 2" <?= selected_option($equipamento->sala, 'Box 2') ?>>Box 2</option>
        <option value="Box 3" <?= selected_option($equipamento->sala, 'Box 3') ?>>Box 3</option>
        <option value="Sala de Raio X" <?= selected_option($equipamento->sala, 'Sala de Raio X') ?>>Sala de Raio X</option>
        <option value="Triagem" <?= selected_option($equipamento->sala, 'Triagem') ?>>Triagem</option>
        <option value="Gabinete Médico" <?= selected_option($equipamento->sala, 'Gabinete Médico') ?>>Gabinete Médico</option>
    </select>
</div>
        ...
        <tr>
            ...
            <td class="py-3 border-0">
                <select name="edit_cons_categoria[]" class="form-select bg-light border-0 shadow-sm" required>
                    ...
                    <option value="Agentes de Desinfeção" <?= ($cons->categoria === 'Agentes de Desinfeção') ? 'selected' : '' ?>>Agentes de Desinfeção</option>
                </select>
            </td>
            <td class="py-3 border-0">
    ...
    ?>
        <tr>
            ...
            <td class="py-3 border-0">
                ...
                <span class="badge bg-secondary bg-opacity-10 text-dark border px-2 py-1 text-decoration-none shadow-sm mt-1">
                    <i class="fa-solid fa-file-pdf text-danger me-1"></i> <?= htmlspecialchars($doc->nome_ficheiro) ?>
                </span>
            </td>
            <td class="py-3 border-0 text-muted small"><?= ($doc->data_validade ? date('d-m-Y', strtotime($doc->data_validade)) : 'N/A') ?></td>
            <td class="py-3 pe-3 border-0 text-end">
